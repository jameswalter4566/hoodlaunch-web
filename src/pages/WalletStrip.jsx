      ...
      <div className="aw-rows">
        {slots.map((s, i) => (
          <div className={'aw-row' + cls(s)} key={s.id}>
            ...
            <button className={'aw-kbtn' + (copied === 's:' + s.id ? ' ok' : '')} title="Copy Solana private key — paste into Phantom" onClick={() => copyKey('s', s)}>{copied === 's:' + s.id ? '✓' : 'SOL'}</button>
            <button className={'aw-kbtn' + (copied === 'e:' + s.id ? ' ok' : '')} title="Copy EVM private key — paste into MetaMask" onClick={() => copyKey('e', s)}>{copied === 'e:' + s.id ? '✓' : 'EVM'}</button>
